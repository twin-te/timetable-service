...
test('既に存在するデータを正しく更新できる', () => {
  const newData = clone(data)
  newData[0].courseId = v4()
  newData[1].name = 'custom course name2'
  return expect(updateRegisteredCourseUseCase(newData)).resolves.toEqual(
    deepContaining(newData)
  )
})

test('無効なデータはエラーになる', () => {
  const brokenData = clone(data)
  brokenData[1].name = undefined
  return expect(updateRegisteredCourseUseCase(brokenData)).rejects.toThrow(
